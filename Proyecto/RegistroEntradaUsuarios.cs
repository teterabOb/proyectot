...
namespace Proyecto
{
    public partial class RegistroEntradaUsuarios: Form
    {
        ...
        public RegistroEntradaUsuarios()
        {
        ...
        {
            DataTable dt = new DataTable();
            string sql = @"select personal.PrimerNombre, PERSONAL.ApellidoPaterno, PERSONAL.rut, CAMION.patente,ESTADORUTA.NombreEstado, SUBCATEGORIA.nombresubcategoria, OBRA.NombreObra, RUTA.DescripcionCarga from RUTA inner join PERSONAL on PERSONAL.idPersonal = RUTA.idPersonal inner join CAMION on CAMION.idCamion = RUTA.idCamion inner join ESTADORUTA on ESTADORUTA.idEstadoRuta = ruta.idEstado inner join SUBCATEGORIA on SUBCATEGORIA.idSubcategoria = ruta.idSubCategoria inner join OBRA on OBRA.idObra = ruta.idObra where ruta.idestado = 1";
            SqlCommand comandosql = new SqlCommand(sql, conn);
            SqlDataAdapter da = new SqlDataAdapter(comandosql);
            da.Fill(dt);
            dv.DataSource = dt;
        ...
        private void RegistroEntradaUsuarios_Load(object sender, EventArgs e)
        {
        ...
        private void txtIdRuta_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            {
                string codigoBarra = txtIdRuta.Text;
